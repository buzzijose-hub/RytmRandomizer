...
**Owns:** `rytm_randomizer/observability/` (new — unified logging + error taxonomy + diagnostic context), every package module's `print()` / `except` / `raise` sites, the `tests/architecture/` observability-conformance tests, the 6 parity test files' subprocess harness, `docs/OBSERVABILITY.md`
**Addresses:** the directive: *ensure world-class observability for troubleshooting; unified handling* — and the related directive *improve how long the test suite takes to execute.* Grounded in the code review: the package currently has **zero `logging` usage**, **8 modules + the monolith using bare `print()`** (554 `print()` calls in the monolith alone), **5 unrelated `*Error` classes** with no common base, and **no diagnostic/trace context** — which is exactly why failures (like a hung subprocess) are invisible. Test-suite *speed* and *progress visibility* are part of the same dev-experience observability problem.
...
**Note — already partly done:** the basic test-observability layer is already committed (`bd3575a`): `subprocess.run(timeout=120, stdin=DEVNULL)` in all 6 parity test files, `pytest-timeout` (any hung test → traceback instead of silent wedge), and `pytest-sugar` (a live progress bar with running pass/fail counts + a % indicator, so a long run is never mistaken for a hang). WS-U step 7 is the *deeper* test-speed fix that builds on that foundation.
...
7. **Test-suite speed — batch the parity subprocesses.** Measured baseline: the 6 Wave-4 parity test files make **~149 `_parity_subprocess` calls**, each spawning a *fresh Python interpreter* that cold-imports the monolith + `mido` (a C extension) + the whole `rytm_randomizer` package (~1–2 s each). That redundant interpreter-startup + import cost is **~85–90% of the ~20-minute full-suite runtime** — it is *not* test logic. The fix: replace the one-subprocess-per-test pattern with **one persistent, warm subprocess per file**. A session/module-scoped fixture lazily spawns a single long-lived interpreter that imports the monolith + package *once*, then reads `assert_parity` requests on stdin and writes per-case results on stdout; each `test_parity_*` function sends its steps to the already-warm process and checks the reply. This keeps pytest's per-test granularity and the exact byte-for-byte parity guarantees (every stdout/MIDI/state comparison the per-test version did, the batched version still does — it is the *transport* that changes, not the *assertions*). Expected: full suite **~20 min → ~3–4 min**. This is a careful refactor of WS-M/WS-N test-harness code (`_HARNESS` / `_parity_subprocess` / `_run_python` in all 6 files) — it must be done *after* a verified-green baseline so "same N tests pass, just faster" is provable, and the per-file warm subprocess must still carry the `timeout` + `stdin=DEVNULL` safety from `bd3575a`.

**Why this matters:** the stall during this very plan's execution was invisible for two full test runs *because there was no observability*. World-class observability is not a nice-to-have — it is the difference between "the process hung, no idea why" and "operation X timed out after N seconds in module Y." Baked into every module, enforced by conformance tests. And a 20-minute suite that only grows as Wave 4 adds tests is its own dev-experience failure — step 7 makes the suite fast enough to actually run on every change.

Acceptance: `rytm_randomizer/observability/` provides unified logging, a single `RytmRandomizerError` taxonomy, and operation tracing; the package's diagnostic `print()` calls are gone (user-facing UI writes deliberately retained); every package `raise`/`except` uses the taxonomy / is specific; a `--debug` flag surfaces full structured logs incl. every MIDI send; `tests/architecture/` enforces the no-bare-print / taxonomy / no-bare-except rules and is gated; `docs/OBSERVABILITY.md` is the troubleshooting guide; the parity subprocesses are batched to one warm interpreter per file (full suite back to single-digit minutes) with the parity guarantees and subprocess safety intact; the full suite is still green at the same test count.
