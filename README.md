...
The live GUI cockpit boundary-readiness report composes the desktop render harness into a passive implementation boundary packet for the future cockpit. It keeps armed hardware behind `python -m rytm_randomizer.app --arm`, keeps passive reports under `python -m rytm_randomizer.cli`, makes Rytm pads 1-12 and Analog Four tracks 1-4 explicit, records the `RYTM_RAND_WS_PORT` documentation targets, and lists optional Tauri/web lockfile and dependency guardrails without launching a GUI, opening MIDI ports, or sending MIDI.

The cockpit send-plan readiness report consumes a saved `CockpitSendPlan` JSON document from `prepare_send_plan` or `send_plan_changed` and tells the operator whether SEND may be enabled, which pads and parameters would move, which pads are locked, what is blocking readiness, and the next action. It remains passive and never prepares, applies, opens ports, or sends MIDI.

The cockpit send-plan rehearsal surface report consumes either that same saved `CockpitSendPlan` JSON or the readiness report JSON and emits deterministic GUI-ready state: review panels, state bindings, disabled SEND/apply controls, acceptance checks, blocked active actions, and passive replay commands. It remains a CLI-only rehearsal surface and never launches a GUI, opens a sidecar, opens MIDI ports, or sends hardware messages.
...
The live GUI controller state report composes that action reducer into deterministic GUI controller/view-model metadata. It includes control-state rows, queued allowed GUI actions, blocked controls, deterministic JSON, and passive replay commands without launching a GUI, dispatching GUI events, mutating a GUI state store, writing files, opening ports, or sending MIDI.

The live GUI playback transcript report composes that controller state into deterministic future-GUI test metadata. It includes playback timeline events, GUI playback assertions, analyzer checkpoints, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, dispatching GUI events, mutating a GUI state store, writing files, opening ports, or sending MIDI.

The live GUI playback validation report composes that transcript into a deterministic validation matrix. It includes future test-harness steps, timeline/assertion/analyzer/safety cases, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, running a test harness, executing commands, recording audio, opening ports, or sending MIDI.

The live GUI test-harness contract report composes that validation matrix into deterministic GUI/audio-analyzer harness metadata. It includes Harness suites, fixture contracts, selector bindings, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, running a test harness, executing commands, writing files, reading audio streams, opening ports, or sending MIDI.

The live GUI test-harness readiness report composes that contract into deterministic GUI/audio-analyzer rehearsal readiness metadata. It includes readiness gates, readiness checks, rehearsal steps, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, running a harness, executing commands, writing files, reading or comparing audio streams, opening ports, or sending MIDI.

The live GUI implementation bridge report composes that readiness packet into deterministic future-GUI wiring metadata. It includes view-model packets, disabled component mounts, fixture bundles, implementation gates, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, starting a renderer, running a harness, executing commands, writing files, reading or comparing audio streams, opening ports, or sending MIDI.

The live GUI desktop blueprint report composes that implementation bridge into a GUI desktop blueprint for the future performance app. It includes desktop shell selection, declarative viewports, regions, widgets, view-model bindings, fixture file hints, acceptance checks, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, starting a renderer, writing files, opening ports, or sending MIDI.

The live GUI desktop app plan report composes that blueprint into a GUI desktop app plan for the future performance app. It includes app shell selection, routes, component file hints, state slices, style tokens, acceptance checks, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, starting a dev server, running a bundler, writing files, opening ports, or sending MIDI.

The live GUI desktop component contract report composes that app plan into a GUI desktop component contract for the future performance app. It includes component props, disabled actions, test selectors, acceptance checks, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, mounting components, dispatching GUI events, writing files, opening ports, or sending MIDI.

The live GUI desktop view-model report composes that component contract into the bindable state shape for the future performance app. It includes component view models, state bindings, disabled action view models, style tokens, acceptance checks, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, mounting components, dispatching GUI events, starting a dev server, writing files, opening ports, or sending MIDI.

The live GUI desktop render contract report composes that view model into renderer-ready metadata for the future performance app. It includes render surfaces, render bindings, style-token bindings, render assertions, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, mounting components, executing a renderer, writing files, opening ports, or sending MIDI.

The live GUI desktop render harness report composes that render contract into the passive test surface for the future performance app. It includes surface harnesses, binding harnesses, style-token checks, harness assertions, blocked actions, deterministic JSON, and passive replay commands without launching a GUI, running browser automation, taking screenshots, writing files, opening ports, or sending MIDI.

Live set card for the current validated Rytm performance layer:

```text
Preflight: run the passive reports first, especially style-performance-arc-live-gui-cockpit-boundary-readiness-report after style-performance-arc-live-gui-desktop-render-harness-report, style-performance-arc-live-gui-desktop-render-contract-report, style-performance-arc-live-gui-desktop-view-model-report, style-performance-arc-live-gui-desktop-component-contract-report, style-performance-arc-live-gui-desktop-app-plan-report, style-performance-arc-live-gui-desktop-blueprint-report, style-performance-arc-live-gui-implementation-bridge-report, style-performance-arc-live-gui-test-harness-readiness-report, style-performance-arc-live-gui-test-harness-contract-report, style-performance-arc-live-gui-playback-validation-report, style-performance-arc-live-gui-playback-transcript-report, style-performance-arc-live-gui-controller-state-report, style-performance-arc-live-gui-action-reducer-report, style-performance-arc-live-gui-interaction-script-report, style-performance-arc-live-gui-analyzer-frame-report, style-performance-arc-live-gui-analyzer-overlay-report, style-performance-arc-live-gui-render-tree-report, style-performance-arc-live-gui-screen-contract-report, style-performance-arc-live-gui-sidecar-session-report, style-performance-arc-live-gui-capture-review-report, style-performance-arc-live-gui-capture-queue-report, style-performance-arc-live-gui-rehearsal-session-report, style-performance-arc-live-gui-analyzer-readiness-report, style-performance-arc-live-analyzer-targets-report, style-performance-arc-live-analyzer-handoff-report, style-performance-arc-live-control-surface-report, style-performance-arc-live-readiness-report, style-performance-arc-live-state-report, style-performance-arc-live-command-deck-report, style-performance-arc-live-transition-timeline-report, style-performance-arc-live-show-export-report, style-performance-arc-live-set-cockpit-report, style-performance-arc-stage-routing-report, and style-performance-arc-stage-rehearsal-state-report.
