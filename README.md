...
The standalone RUSH01 scripts are passive compile/report surfaces. Plan
compilation is dry-run by default, and operator-specific output is written
under the gitignored `output/local/` directory. They do not construct a MIDI
provider, open a port, or transmit MIDI.

Real RUSH01 learning and plan application are app-owned operations under
`python -m rytm_randomizer.app --arm`. Learning opens one exact-name input and
sends nothing. Application requires one selected device, an exact configured
output, and the feature-specific `--confirm-rush01-midi-send` guard. The app
routes the reviewed plan through the canonical `ArmedApplySession` seam; it sends
only reviewed CC messages and never sends SysEx, transport, Program Change,
pattern, song, project, kit-write, or save messages. Direct offline KIT SysEx
generation remains blocked while saved-kit semantic mappings are unverified,
and the local-model copilot has no hardware path. See the
[CLI reference](docs/CLI_REFERENCE.md#rush01-compile-learn-and-apply).
