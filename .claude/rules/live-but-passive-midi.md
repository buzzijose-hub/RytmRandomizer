...
- **Outputs are armed — on the cockpit / live surface.** Every **outbound** transmit reachable from the cockpit (desktop app, WS sidecar, `cockpit/**`) routes through the `senders` ArmedApply seam, behind an explicit **in-UI arm action + per-action confirmation**. There is no "transmit because a port happened to be open" path there.
  ...
  **Scope, stated honestly:** this is *not* yet a repo-wide invariant. Exactly **one** module is exempt: the legacy V1.34 CLI entry point `rytm_randomizer/app.py`, which opens its own real output ports at ten `open_output(...)` sites behind the CLI's own `--arm` discipline. It is exempt only because it is named in the shrinking allowlist `_LEGACY_V134_TRANSMIT_MODULES` in `tests/architecture/test_armed_entry_points.py`; the allowlist, not this prose, is the enforcement. Until it comes off, do not describe the ArmedApply seam as the only outbound path in the repository — describe it as the only outbound path *for the cockpit*.

  **Why `app.py` cannot simply be re-pointed at the seam.** Its armed sends go through `midi_io.send_cc`, which per message builds a `mido.Message`, records `record_cc_sent`, and then sleeps `MIDI_MESSAGE_SETTLE_SECONDS` (0.02s) so the hardware sees discrete messages. The seam's `apply` loop writes bare `(channel, control, value)` triples with **no pacing and no metric**, so migrating today would silently drop the settle delay on every armed CLI send — an observable hardware-behaviour change the V1.34 parity contract forbids. Three of the ten sites (`_run_arm`, `--rytm-12-pad-shell`, `--rytm-snapshot-shell`) also hand the port to a long-lived interactive shell that streams for minutes, which the seam's render-a-plan-then-burst `apply` does not model. Closing this means first teaching the seam paced delivery and a streaming mode — a change to the seam's own behaviour, and therefore its own reviewed change.

  **`shell.py` is no longer exempt** (removed 2026-07-31). It never transmitted: it receives an already-opened port as an injected `Sender` and writes only through `midi_io`. Its two former "sites" were a docstring and a comment *quoting* the monolith's `with mido.open_output(...)` line — the scanner counted prose as code. The scan now blanks comments and string literals before matching, and `test_retired_legacy_modules_still_do_not_transmit` pins that `shell.py` stays transmit-free.
...
1. **Route all *new* outbound MIDI through the `senders` ArmedApply seam** (`rytm_randomizer/senders/guarded.py` + `senders/hardware.py`). Engines, runners, and plans call `midi_io.send_cc` on a **dependency-injected** `Sender` protocol; whether that sender is a mock or a real port is decided at the armed boundary, never at the call site. The pre-existing `app.py` transmit sites are grandfathered via the allowlist (see the scope note above) — new code gets no such exemption.
2. **Keep the transmit whitelist shrinking.** `_ALLOWED_TRANSMIT_MODULES` in `tests/architecture/test_armed_entry_points.py` is the frozen set of modules allowed to construct real output ports or define the hardware send: the two boundary modules (`mido_provider.py`, `real_midi_adapter.py`), the seam modules (`senders/hardware.py`, `senders/guarded.py`, `senders/armed_apply.py`, `midi_io.py`), and the one grandfathered V1.34 entry point (`app.py`). Adding an entry requires reviewer sign-off recorded in the PR body; the direction is one-way — `_LEGACY_V134_ALLOWLIST_MAX_SIZE` may only be lowered, and a module retired from the list is pinned transmit-free by `test_retired_legacy_modules_still_do_not_transmit` so it cannot quietly regain an exemption. The list is the mechanical authority for what is exempt — keep this rule's prose in sync with it.
