...
2. **No `mido` at module top level** anywhere in the package. `mido` may
   ...
   `midi_io.py`.
3. **No monolith import.** No file under `rytm_randomizer/` imports
   `rytm_hybrid_randomizer_v134`. Only tests may.
...
6. **Parity discipline.** If `engines/*`, `group_runner.py`, or
   `scene_runner.py` changed, confirm the monolith reference file was NOT
   edited.
