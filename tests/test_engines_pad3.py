...
* The subprocess seeds the stdlib ``random`` module identically before driving
  ...
  MIDI messages for each, and asserts equality *inside* the subprocess.
...
# state dicts, and asserts byte-parity. ``steps`` is a list of (fn_name, args)
# tuples; if the first element is the literal string "load", the monolith's
# ``load_group_anchors`` runs first (and the engine is seeded from the exact
# group dicts it produced) so the 4-pad-group guard is satisfied identically.
_HARNESS = '''
...
def test_parity_load_pad3_mode_each_mode():
    for mode in ["anchor", "lp1", "bandpass", "wave", "scifi"]:
        for seed in (1, 7, 42):
            _parity_subprocess(
...
def test_parity_mutate_current_pad3_mode():
    ...
    # anchor-mode mutation (generic zone branch: body/lfo/morph)
    for seed in (1, 2, 5, 11, 42):
        _parity_subprocess(
