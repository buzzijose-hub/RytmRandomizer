...
sees discrete messages rather than one burst. The ArmedApply seam now supports
an injected post-send observer so bounded plans such as RUSH01 can preserve
per-message pacing and metrics while using its canonical arm/confirm/apply
lifecycle. Three legacy sites (``_run_arm``, ``--rytm-12-pad-shell``,
...
Closing the remaining legacy exception requires a streaming-session mode and
then migrating those interactive sites in their own reviewed change.
...
# ``MIDI_MESSAGE_SETTLE_SECONDS``). The ArmedApply seam supports a post-send
# observer for bounded plans, but it does not yet model the monolith's
# long-lived streaming shell sessions; see the module docstring's
# "Why app.py is still here" note.
#
