...
_ALLOWED_ROOT_DIRS: Final[frozenset[str]] = frozenset(
    {
        ...
        "calibration",  # public observed-only MIDI calibration evidence
        "captures",  # raw operator-supplied SysEx capture evidence
        "config",  # public config examples; machine-local values stay ignored
        "desktop",  # Tauri shell + React cockpit
        "docs",  # documentation, plans, diagrams
        "installer-assets",  # packaging icons / metadata
        "output",  # frozen evidence; operator output is isolated under ignored local/
        "rytm_randomizer",  # the Python package
        "specs",  # source semantic specifications compiled into passive plans
