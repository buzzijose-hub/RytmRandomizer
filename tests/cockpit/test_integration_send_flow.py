...
ack carries the new snapshot id, and four events fire in order:
...
def _arm_candidate(ws: object, profile_id: str = "scene-industrial", depth: float = 0.45) -> None:
    """Helper: select a profile + set a depth so a candidate is staged for SEND."""
...
def test_send_emits_events_in_documented_order(cockpit_ws: object) -> None:
    """SEND event order is snapshot → history → preview-cleared → session_status."""
...
def test_send_history_grows_one_entry_per_send(cockpit_ws: object) -> None:
    ...
    events1 = drain_events(cockpit_ws, 4)
    ...
    events2 = drain_events(cockpit_ws, 4)
