...
def test_analog_four_runtime_report_help_exits_zero():
    result = run_cli("analog-four-runtime-report", "--help")

    assert result.returncode == 0
    assert "RytmRandomizer passive CLI: analog-four-runtime-report" in result.stdout
    assert "--track <1-4>" in result.stdout
    assert "no MIDI sending" in result.stdout
    assert result.stderr == ""
...
def test_twelve_pad_rytm_runtime_validation_guide_help_exits_zero():
...
def test_dual_machine_kit_bank_readiness_report_help_exits_zero():
    result = run_cli("dual-machine-kit-bank-readiness-report", "--help")
...
def test_analog_four_saved_offset_mapping_promotion_report_help_exits_zero():
    result = run_cli("analog-four-saved-offset-mapping-promotion-report", "--help")

    assert result.returncode == 0
    assert (
        "RytmRandomizer passive CLI: analog-four-saved-offset-mapping-promotion-report"
        in result.stdout
    )
...
def test_dual_machine_mock_bridge_report_help_exits_zero():
    ...
    assert "--target analog-four --depth <micro|groove|strong>" in result.stdout
    assert "may omit the Rytm path and --slot" in result.stdout
...
def test_dual_machine_lane_validation_guide_help_exits_zero():
    result = run_cli("dual-machine-lane-validation-guide", "--help")
    ...
    assert "--all-lanes" in result.stdout
    assert "--target <rytm|analog-four|both>" in result.stdout
