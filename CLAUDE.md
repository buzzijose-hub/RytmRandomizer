...
8. **Live-but-Passive MIDI.** Launch may enumerate ports and open MIDI **inputs** freely (read-only listening — never interrupts device sound output). Every **cockpit** output/transmit routes through the `senders` ArmedApply seam behind an explicit in-UI arm + per-action confirmation, and never auto-re-arms after reconnect. This is a cockpit/live-surface guarantee, **not repo-wide**: the legacy V1.34 entry points `app.py` (11 `open_output` sites) and `shell.py` (2) still open their own output ports under the CLI's `--arm` discipline and are exempt via the named shrinking allowlist `_ALLOWED_TRANSMIT_MODULES` in `tests/architecture/test_armed_entry_points.py` (WS-4 removes them). Persistent kit/sound writes are **refused** (`KitMutationUnsupportedError`) because real capture-before-write and restore do not exist; only RAM-only live-dial CC/NRPN sends are permitted. Enforced by `tests/architecture/test_armed_entry_points.py` + `test_repo_root_perimeter.py`. See [`.claude/rules/live-but-passive-midi.md`](.claude/rules/live-but-passive-midi.md).
