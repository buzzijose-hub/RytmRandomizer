...
anchor/state utility gap category:
...
- `B`, `E`, `W`, and `U` metadata expansion plan accepted and implemented as
...
The accepted `B`, `E`, `W`, and `U` plan has been implemented as passive
metadata only.

Added passive `STATE_UTILITY_COMMANDS` entries:
...
- `python -m rytm_randomizer.cli inspect-command B`
- `python -m rytm_randomizer.cli inspect-command E`
- `python -m rytm_randomizer.cli inspect-command W`
- `python -m rytm_randomizer.cli inspect-command U`
...
- the first targeted scaffold run failed because `STATE_UTILITY_COMMANDS` did
  not exist yet
- minimal passive metadata was added
- targeted scaffold, command lookup, registry report, and passive CLI checks
  passed
