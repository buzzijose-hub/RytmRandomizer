"""Passive Rytm style mutation render-plan report."""
...
from ..devices.strategies.analog_rytm_style_mutation_render_plan import (
...
REPORT_TITLE: Final[str] = "RytmRandomizer passive Rytm style mutation render plan"
SOURCE_MODULE: Final[str] = "reports.rytm_style_mutation_render_plan"
SAFETY_LINES: Final[tuple[str, ...]] = (
    "passive/read-only",
    "render-plan metadata only",
    "no MIDI rendering",
    "no MIDI sending",
    "no port opening",
...
_USAGE: Final[str] = (
    "rytm-style-mutation-render-plan-report usage: "
...
def _event_text(event: RytmStyleMutationRenderEvent) -> str:
    return (
        f"    - {event.zone} | {event.parameter} | target {event.target_value} "
        f"| window {event.window_low}-{event.window_high} | safe {event.low}-{event.high} "
        f"| depth {event.mutation_depth} | direction {event.target_direction}"
    )


def _body_lines(plan: RytmStyleMutationRenderPlan) -> list[str]:
    lines = [
        f"Kit: {plan.kit_name}",
        f"Slot: {plan.slot}",
        ...
        f"Render ready: {plan.render_ready}",
        "Summary:",
        f"- Ready pads: {plan.ready_pad_count}",
        f"- Blocked pads: {plan.blocked_pad_count}",
        f"- Render events: {plan.render_event_count}",
        "Pads:",
    ]
    for pad in sorted(plan.pads_by_pad):
        ...
        lines.extend(
            [
                ...
                f"  Current machine: {pad_plan.current_machine_key or 'unknown'}",
                f"  Profile key: {pad_plan.profile_key or 'none'}",
                f"  Route ready: {pad_plan.route_ready}",
                f"  Render ready: {pad_plan.render_ready}",
                f"  Reason: {pad_plan.readiness_reason}",
                f"  Favored zones: {_join(pad_plan.favored_zones)}",
            ]
        )
        if not pad_plan.render_events:
            lines.append("  Render events: none")
        else:
            lines.append("  Render events:")
            lines.extend(_event_text(event) for event in pad_plan.render_events)
    lines.append(SAFETY_SECTION_HEADER)
    lines.extend(f"- {line}" for line in SAFETY_LINES)
    return lines


def format_rytm_style_mutation_render_plan_report(
    snapshot_or_plan: RytmKitSnapshot | RytmStyleMutationRenderPlan,
    *,
    style_key: str,
    discovery_amount: int = DEFAULT_STYLE_DISCOVERY_AMOUNT,
) -> list[str]:
    """Return deterministic operator-facing style render-plan lines."""

    plan = (
        snapshot_or_plan
        if isinstance(snapshot_or_plan, RytmStyleMutationRenderPlan)
        else plan_rytm_style_mutation_render_plan(
            snapshot_or_plan,
            style_key,
...
def _event_json(event: RytmStyleMutationRenderEvent) -> dict[str, object]:
    return {
        "zone": event.zone,
        "parameter": event.parameter,
        "low": event.low,
        "high": event.high,
        "mutation_depth": event.mutation_depth,
        "target_bias": event.target_bias,
        "target_direction": event.target_direction,
        "target_value": event.target_value,
        "window_low": event.window_low,
        "window_high": event.window_high,
    }


def to_rytm_style_mutation_render_plan_json(
    plan: RytmStyleMutationRenderPlan,
) -> dict[str, object]:
    """Return deterministic machine-readable Rytm style render-plan metadata."""
    ...
    return {
        ...
        "render_event_count": plan.render_event_count,
        "pads": [
            {
                "pad": pad_plan.pad,
                ...
                "render_ready": pad_plan.render_ready,
                "readiness_reason": pad_plan.readiness_reason,
                "favored_zones": list(pad_plan.favored_zones),
                "render_event_count": pad_plan.render_event_count,
                "render_events": [_event_json(event) for event in pad_plan.render_events],
            }
    ...
    *,
    ...
    try:
        ...
        if json_output:
            sys.stdout.write(
                json.dumps(
                    to_rytm_style_mutation_render_plan_json(plan),
                    indent=2,
                    sort_keys=True,
                )
            )
...
RYTM_STYLE_MUTATION_RENDER_PLAN_CLI_COMMAND: Final[CliCommand] = CliCommand(
    name="rytm-style-mutation-render-plan-report",
    summary="Print passive Rytm style mutation render-plan metadata for a SysEx file.",
    args_parser=_parse_cli_args,
    handler=_handle_cli_report,
    error_formatter=_format_cli_error,
)

register(RYTM_STYLE_MUTATION_RENDER_PLAN_CLI_COMMAND)
