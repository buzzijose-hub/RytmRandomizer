"""Passive live GUI interaction-script packet for analyzer-frame state."""
...
SOURCE_MODULE: Final[str] = "reports.live_gui_interaction_script"
...
def _blocked_actions(script_status: str) -> tuple[str, ...]:
    actions = [
        "no GUI launch",
        "no GUI event dispatch",
...
def _replace_command(command: str, *, interaction_label: str) -> str:
...
def _replay_commands(
    ...
    *,
    ...
    return (
        _replace_command(frame.replay_commands[0], interaction_label=interaction_label),
...
def build_style_performance_arc_live_gui_interaction_script_from_frame(
    ...
    *,
    ...
    return StylePerformanceArcLiveGuiInteractionScriptReport(
        ...
        replay_commands=_replay_commands(frame, interaction_label=normalized_label),
    )
...
def _step_lines(step: StylePerformanceArcLiveGuiInteractionStep) -> list[str]:
    return [
        f"- {step.order}. {step.key}: {step.action_type} / {step.state}",
        f"  Target: {step.target_key}",
        f"  Test id: {step.test_id}",
        f"  Source: {step.source}",
        f"  Action: {step.operator_action}",
    ]


def _binding_lines(binding: StylePerformanceArcLiveGuiControlBinding) -> list[str]:
    enabled = "enabled" if binding.enabled else "disabled"
    return [
        f"- {binding.key}: {binding.control_type} / {enabled}",
        f"  Label: {binding.label}",
        f"  State: {binding.state}",
        f"  Reason: {binding.reason}",
        f"  Source: {binding.source}",
        f"  Test id: {binding.test_id}",
        f"  Action: {binding.operator_action}",
    ]


def format_style_performance_arc_live_gui_interaction_script_report(
    report: StylePerformanceArcLiveGuiInteractionScriptReport,
) -> list[str]:
    """Return deterministic passive live GUI interaction-script lines."""

    lines = [
        "Live GUI interaction script summary:",
        f"- Script version: {report.script_version}",
        f"- Script id: {report.script_id}",
        f"- Interaction label: {report.interaction_label}",
        f"- Script status: {report.script_status}",
    ...
    for step in report.interaction_steps:
        lines.extend(_step_lines(step))
    lines.append("GUI control bindings:")
    for binding in report.control_bindings:
...
def _parse_cli_args(argv: Sequence[str]) -> dict[str, object]:
    ...
    while remaining:
        ...
        if option == "--interaction-label":
            ...
            if option != "--json":
                frame_args.append(_pop_option_value(remaining))
    ...
    *,
    ...
    try:
        ...
        if json_output:
            sys.stdout.write(
                json.dumps(
                    to_style_performance_arc_live_gui_interaction_script_json(report),
                    indent=2,
                    sort_keys=True,
                )
            )
...
STYLE_PERFORMANCE_ARC_LIVE_GUI_INTERACTION_SCRIPT_CLI_COMMAND: Final[CliCommand] = CliCommand(
    name="style-performance-arc-live-gui-interaction-script-report",
    summary="Compose passive analyzer frame into GUI interaction metadata.",
    args_parser=_parse_cli_args,
    handler=_handle_cli_report,
    error_formatter=_format_cli_error,
)

register(STYLE_PERFORMANCE_ARC_LIVE_GUI_INTERACTION_SCRIPT_CLI_COMMAND)
