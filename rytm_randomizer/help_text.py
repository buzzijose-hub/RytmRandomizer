...
Behavior:
  ...
  candidate readiness summary, prints device-specific implementation
  boundaries, then prints target-scoped next passive validation commands for
  Rytm-only, Analog-Four-only, and both-machine lane checks, mapping-session
  rehearsal, and A4 manifest review. It does not request dumps, receive live
  SysEx, send MIDI, open ports, write SysEx, or touch hardware.
