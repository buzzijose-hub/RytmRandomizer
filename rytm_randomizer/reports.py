...
_REPORT_BUILDERS = {
    "registry": build_registry_report,
    "active_boundary": build_active_boundary_report,
    "mock_mapper": build_mock_mapper_report,
    "runtime_plan": build_runtime_plan_report,
    "anchor_profile": build_anchor_profile_report,
    "behavior_parity_coverage": build_behavior_parity_coverage_report,
    "mock_runtime_active_bridge": build_mock_runtime_active_bridge_report,
}

_REPORT_FORMATTERS = {
    "registry": format_registry_report,
    "active_boundary": format_active_boundary_report,
    "mock_mapper": format_mock_mapper_report,
    "runtime_plan": format_runtime_plan_report,
    "anchor_profile": format_anchor_profile_report,
    "behavior_parity_coverage": format_behavior_parity_coverage_report,
    "mock_runtime_active_bridge": format_mock_runtime_active_bridge_report,
}

_REPORT_SUMMARIZERS = {
    "registry": summarize_registry_report,
    "active_boundary": summarize_active_boundary_report,
    "mock_mapper": summarize_mock_mapper_report,
    "runtime_plan": summarize_runtime_plan_report,
    "anchor_profile": summarize_anchor_profile_report,
    "behavior_parity_coverage": summarize_behavior_parity_coverage_report,
    "mock_runtime_active_bridge": summarize_mock_runtime_active_bridge_report,
}

REPORT_KEYS = tuple(_REPORT_BUILDERS)


def build_report(report_key):
    """Build the passive, in-memory report for ``report_key``."""
    try:
        builder = _REPORT_BUILDERS[report_key]
    except KeyError:
        raise KeyError(f"unknown report key: {report_key!r}") from None
    return builder()


def format_report(report_key, report=None):
    """Format the report identified by ``report_key`` as deterministic lines.

    Pass an already-built ``report`` to format it directly; otherwise the
    report is built fresh from its registered builder.
    """
    try:
        formatter = _REPORT_FORMATTERS[report_key]
    except KeyError:
        raise KeyError(f"unknown report key: {report_key!r}") from None
    return formatter(report)


def summarize_report(report_key, report=None):
    """Return the compact summary for the report identified by ``report_key``."""
    try:
        summarizer = _REPORT_SUMMARIZERS[report_key]
    except KeyError:
        raise KeyError(f"unknown report key: {report_key!r}") from None
