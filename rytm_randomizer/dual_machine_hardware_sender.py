...
does not open ports or choose devices; callers inject an already-open output
port. The guard accepts only a ready, single-target ``DualMachineActiveSendPlan``
...
def format_dual_machine_hardware_send_report(
    ...
    lines.extend(
        [
            "Guard policy:",
            "- single-target hardware send only",
            "- requires --arm",
