...
class PadDelta:
    ...
    def from_dict(cls, data: Mapping[str, object]) -> Self:
        ...
        assert isinstance(proposed_obj, Mapping)
        assert isinstance(changed_obj, (list, tuple, frozenset, set))
        return cls(
...
class MutationCandidate:
    ...
    def __post_init__(self) -> None:
        ...
        if not (_DEPTH_MIN <= self.depth <= _DEPTH_MAX):
            raise ValueError(
                f"depth must be in [{_DEPTH_MIN}, {_DEPTH_MAX}]; got {self.depth}"
            )
        ...
        if self.estimated_midi_msgs < 0:
            raise ValueError(
                f"estimated_midi_msgs must be >= 0; got {self.estimated_midi_msgs}"
            )
    ...
    def from_dict(cls, data: Mapping[str, object]) -> Self:
        ...
        assert isinstance(deltas_obj, (list, tuple))
        return cls(
