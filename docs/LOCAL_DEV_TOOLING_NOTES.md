...
| `RYTM_AL16_MANIFEST_REFRESH` | unset | `1` lets `scripts/refresh_al16_evidence_manifest.py` rewrite the `generator_dependency_sha256` map in the committed AL02 evidence manifest. Needed whenever an edit touches a file pinned in `AL16_GENERATOR_DEPENDENCIES` — including a purely additive one, such as registering a new device family, since `rytm_randomizer/devices/__init__.py` is on that list. The script refuses to run if the pinned key set changed (that is a deliberate contract change) or if the manifest records a real exported artifact (`output_sha256` set), and it prints the new manifest SHA to paste into `_COMMITTED_EVIDENCE_HASHES` in `tests/test_al16_rytm_export.py`. Never hand-edit the manifest. |
