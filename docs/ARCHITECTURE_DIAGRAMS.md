...
flowchart LR
    ...
    RytmMappingService --> RytmMappingInputs["recipe SHA + gap-manifest SHA<br/>recipe identifier + reference SHA"]
    RytmMappingService --> RytmCanonicalFacts["data/analog_rytm_kit_layout.py<br/>cockpit/data/rytm_parameter_map.py"]
...
flowchart LR
    ...
    subgraph Reports["Passive reports"]
        ...
        AL16RytmMappingEvidence["al16-rytm-mapping-evidence --reference K --configured C --recipe R --gap-manifest G --report O"]
...
- The registered `al16-rytm-mapping-evidence` command compares initialized and manually configured local Rytm kit dumps against the R1 gap manifest. It verifies recipe and manifest hashes, deterministic recipe identity, and initialized-reference identity before analysis; candidate locations are restricted to canonical layout and parameter-map facts. The command writes review-only JSON, records structured operations/metrics, and cannot promote mappings, write SysEx, or reach hardware.
