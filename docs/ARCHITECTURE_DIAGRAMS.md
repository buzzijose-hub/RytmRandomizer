...
- Current package: `rytm_randomizer/` - 26 top-level Python files + 14 subpackages = 384 total Python modules. The 14 subpackages: `behavior/`, `cockpit/`, `data/`, `devices/` (with nested `devices/strategies/`), `dual_machine/`, `engines/`, `guardrails/`, `local_ai/`, `observability/`, `reports/`, `senders/`, `snapshot/`, `state/`, `style_analysis/`.
...
| Style analysis | `rytm_randomizer/style_analysis/{extractor,runtime_types,feature_report,library,blueprint,reference_audio_atlas,analog_four_patch_genome,analog_four_patch_inference,analog_four_patch_learning,analog_four_patch_corpus,analog_four_patch_send_plan,analog_four_patch_render_rank,analog_four_patch_codesigner,analog_four_patch_refinement,analog_rytm_recipe_inference,audio_patch_dna}.py` |
| Cockpit export | `rytm_randomizer/cockpit/export/{file_export_contracts,analog_four_export_contracts,analog_four_cli,analog_four_kit,al16_rytm_cli,al16_rytm_kit,al16_rytm_mapping_closure,al16_rytm_mapping_closure_cli,rio145_cli,rio145_codec,analog_four_patch_batch,analog_four_patch_batch_cli,analog_four_patch_batch_codec,analog_four_patch_batch_contracts,analog_four_patch_batch_publication,analog_four_patch_batch_reader,analog_four_patch_render_rank,analog_four_patch_render_rank_cli,analog_four_patch_refinement,analog_four_patch_refinement_cli,cli_options,writer}.py` plus the profile-model serialization/signing/verifier modules. |
| Tests | `tests/test_*.py`, `tests/cockpit/test_*.py`, `tests/architecture/test_*.py`, `tests/fixtures/{analog_four_saved_kit,rio145,v134_parity}/`, `tests/_parity_worker.py`, `tests/conftest.py` |
...
flowchart TB
    ...
    Package["Modular package<br/>rytm_randomizer/<br/>(14 subpackages, 384 modules)"]
...
flowchart TB
    ...
    subgraph DataLayer["Data layer (single source of truth)"]
        ...
        DataA4SavedKitLayout["data/analog_four_saved_kit_layout.py<br/>A4 saved-kit wire-layout facts"]
    ...
    subgraph ReportsPkg["reports/ subpackage<br/>(was reports.py)"]
        ...
        RReferenceAudioAtlas["reference_audio_atlas.py<br/>passive bounded long-form atlas<br/>+ registered CliCommand"]
        ...
        RLocalModelCopilot["local_model_copilot.py<br/>passive local-AI packets<br/>+ registered CliCommand"]
    end

    subgraph LocalAiPkg["local_ai/ subpackage"]
        LAProvider["provider.py<br/>DTOs + validation helpers"]
        LALocalModel["local_model.py<br/>explicit local executable adapter"]
        LARag["rag.py<br/>docs/MIDI context packets"]
        LAMutation["mutation_intent.py<br/>staged mutation schema"]
    end
    ...
    subgraph StyleAnalysis["style_analysis/"]
        ...
        SAReferenceAudioAtlas["reference_audio_atlas.py<br/>bounded window scan + deterministic moment selection"]
    ...
    ReportsPkg --> LocalAiPkg
    LocalAiPkg --> ObservabilityPkg
    StyleAnalysis --> DataLayer
    RReferenceAudioAtlas --> SAReferenceAudioAtlas
    SAReferenceAudioAtlas --> SAExtractor
    SAReferenceAudioAtlas --> SAPatchGenome
    App --> SendersPkg
```
...
flowchart TB
    ...
    subgraph A4Impls["Analog Four impls (devices/strategies/)"]
        A4Layout["data/analog_four_saved_kit_layout.py<br/>canonical wire facts"]
        A4Codec["analog_four_saved_kit_codec.py<br/>shared validate/decode/encode"]
...
flowchart LR
    ...
    subgraph ReportCmds["Direct read-only report commands"]
        ...
        ReferenceAudioAtlasReport["reference-audio-atlas-report"]
        ...
        LocalModelCopilotReport["local-model-copilot-report<br/>passive local-AI packets; --ask-local-model explicit"]
    ...
    subgraph RIO145LocalCmds["Passive RIO145 dual-device local-file commands"]
        RIO145Inspect["rio145-inspect-sysex / rio145-diff-sysex<br/>file inspection + semantic/native diff"]
        RIO145Validate["rio145-validate-roundtrip / rio145-validate-return --device ...<br/>codec + target-return validation"]
        RIO145Build["rio145-build-kit --device ...<br/>typed recipe -> deterministic saved KIT"]
        RIO145Oxi["rio145-export-oxi-manifest<br/>four programs; no native device patterns"]
    end
    ...
    RIO145LocalCmds --> RIO145Codec["cockpit/export/rio145_codec.py<br/>canonical envelope + A4/Rytm saved-KIT codecs"]
    RIO145Codec --> NativeObject["snapshot/elektron_native_object.py<br/>file-only native-object adapter"]
...
flowchart TB
    ...
    subgraph CodexRedo["Landed (was: PR #36 redo target)"]
        ...
        Manifest["data/analog_four_saved_kit_layout.py<br/>(saved-kit frame constants + sizes)<br/>+ strategy compatibility manifest;<br/>A4-specific, not in dual_machine/"]
    end
...
flowchart LR
    ...
    subgraph Reports["Passive reports"]
        ...
        ReferenceAudioAtlas["reference-audio-atlas-report --audio A [--window-seconds N] [--hop-seconds N] [--max-windows N] [--moments N] [--json]"]
        ...
        RIO145Commands["rio145-*<br/>inspect / diff / roundtrip / A4+Rytm build+return / OXI manifest"]
    ...
    CliRegistry -->|"registered passive command:<br/>reference-audio-atlas-report"| CLI
    ...
    CliRegistry -->|"registered passive local-file commands:<br/>six rio145-* commands"| CLI
...
- **Stdlib only.** `hmac` + `hashlib` (HMAC-SHA256), `zlib` (CRC32),
  `tempfile.mkstemp` + `os.fsync` + platform-native atomic publication
