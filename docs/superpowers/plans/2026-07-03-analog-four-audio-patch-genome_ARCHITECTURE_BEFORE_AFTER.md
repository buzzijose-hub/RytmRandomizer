...
| Capability | State |
|---|---|
| Audio analysis | A spawned native child analyzes one immutable recording; the parent converts abnormal exit into `inference_failed`, cleans private staging, and produces typed deterministic A4 inference when decoding succeeds. |
...
| Operator path | The canonical passive batch emits exactly four `.syx` candidates, complete DNA sidecars, and a hashed manifest; the verified reader feeds mock dry-run or the sole confirmed `app --arm` CC/NRPN path, while recorded renders feed a passive acoustic ranker. |
...
- `RealMidiOutputProvider` and the canonical MIDI event-kind vocabulary are
  shared across app, reader, and sender instead of being redefined by the A4
  feature.
...
- Kept all provider construction and real CC/NRPN delivery behind
  `python -m rytm_randomizer.app --arm`; local saved-kit SysEx writing is file
  generation, not MIDI transfer.
