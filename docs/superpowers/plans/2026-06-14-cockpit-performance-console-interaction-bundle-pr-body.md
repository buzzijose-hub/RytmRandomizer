...
Adds local-only rehearsal interactions to the passive Cockpit Performance Console so the cinematic console is no longer just a static packet viewer. Operators can select a style crate, queued move, snapshot, and preview depth, then create an in-memory dry-run summary and local journal take without dispatching any command or touching MIDI.

## What changed

- Added component-local rehearsal state to `desktop/web/src/cockpit/PerformanceConsole.tsx` for selected crate, queued move, snapshot, preview depth, local dry-run result, and local journal entries.
- Made style crate, queue move, and snapshot cards locally selectable while preserving disabled real hardware actions.
- Added a local rehearsal preview, local dry-run summary, and local mutation journal panel.
- Added CSS for selected/focused local controls and local summary panels.
- Added Vitest coverage for pointer selection, keyboard snapshot selection, empty deck fallback behavior, and hardware-send safety.
- Updated `README.md`, `docs/STATUS.md`, and the implementation plan.

## Why this matters

This is the next large Cockpit step after the cinematic HUD work: it lets Jose rehearse live intent inside the GUI without making any promise that the frontend can send to hardware yet. The safety line stays bright: this PR does not add WebSocket dispatch, Tauri commands, sidecar commands, MIDI port opening, hardware arming, queued command execution, snapshot mutation, or MIDI sends.
...
python -m pytest tests\architecture\test_plan_doc_status_truth.py -q
python -m ruff check .
...
- [x] **Gate 12** - constants: new frontend constants are local immutable `const` values; no Python module constants added.
- [x] **Gate 13** - env var docs: no new environment variables.
- [x] **Gate 14** - maintainability review: state is component-local, derived from existing packet types, and bounded to passive UI rehearsal.
...
Plan doc: `docs/superpowers/plans/2026-06-14-cockpit-performance-console-interaction-bundle.md`

## Reviewer notes

The active buttons added here are intentionally local-only. They update React state and visible rehearsal evidence only; they do not call a cockpit client, WebSocket, Tauri invoke, sidecar command, MIDI adapter, send planner, or queue executor.
