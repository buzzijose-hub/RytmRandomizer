...
Adds the passive style-target-to-Rytm-snapshot bridge: style profile data now has normalized target vectors, operators can inspect those targets from the passive CLI, and a new mock-safe Rytm style snapshot routing report explains which pads in a captured kit are ready to mutate toward a style goal without sending MIDI or touching hardware.
...
- [x] No new dead code: vulture clean on touched style/routing modules and tests.
...
- [x] Gate 4 - no new dead code: vulture clean on touched style/routing files.
...
- [x] Gate 18 - architecture-doc + diagram freshness: architecture diagrams updated for new report/strategy/CLI surfaces.
