...
_CODEX_HANDOFF = (
    ...
    "covering every dimension. The skill's 'Execution model' section is "
    "the authoritative dimension table; spawn one agent per dimension in "
    "it — at least: architecture / import-direction; house style / type "
    "hygiene; parity + test hygiene; side effects + mido leakage; "
    "observability; abstraction reuse / genericization (Step 7 — could the "
    "new code be generalized, or does an existing abstraction such as the "
    "Device Protocol, snapshot/envelope helpers, the generic senders, "
    "cli_registry, data/, observability/metrics, or the report formatter "
    "already cover it); docs + diagram freshness (Step 8 — are "
    "docs/ARCHITECTURE.md and docs/ARCHITECTURE_DIAGRAMS.md updated for any "
    "architecture-surface change, and do the counts the docs quote still "
    "match); and string-literal dispatch / env vars / maintainability / "
    "execution shape / learning capture (Gates 10, 13, 14, 15, 16). Then "
    "synthesize "
